...
# documents = ["Ceci est un document est d'exemple.", "Ce document est un autre exemple.", "Document d'exemple pour les tests."]
# corpus = Corpus("Exemple")
# for i, texte in enumerate(documents):
#     corpus.add(ArxivDocument(titre=f"Document {i+1}", texte=texte, auteur="Auteur"))

# search_engine = SearchEngine(corpus)

# # Demander à l'utilisateur d'entrer des mots clés
# mots_cles = input("Entrez des mots clés séparés par des virgules: ").lower().split(',')

# # Nettoyer les mots clés
# mots_cles = [mot.strip() for mot in mots_cles]

# # Effectuer la recherche
# result_df = search_engine.search(mots_cles, n_docs=3)
